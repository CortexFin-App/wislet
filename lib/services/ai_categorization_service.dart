...
class AICategorizationService {
  ...
  Future<fin_category.Category?> suggestCategory(
      {required String description, required int walletId}) async {
    if (!_isInitialized) {
    ...
    return await categoriesEither.fold((failure) async => null,
        (allCategories) async {
      final categoriesByName = {
        for (var cat in allCategories) cat.name.toLowerCase(): cat
      };
      final categoriesById = {
        for (var cat in allCategories) if (cat.id != null) cat.id!: cat
      };
      ...
      for (var entry in _brandCategoryMap.entries) {
        ...
        if (lowerCaseDescription.contains(brand)) {
          ...
          final type = (categoryName == 'Зарплата' ||
                  categoryName == 'Подарунки та Допомога' ||
                  categoryName == 'Додатковий Дохід' ||
                  categoryName == 'Інвестиції')
  ...
  static const Map<String, String> _baseBrandMap = {
    ...
    'подорожник': 'Здоров\'я та медицина',
    ...
    'макдональдз': 'Кафе, Бари, Ресторани',
  ...
  static const Map<String, List<String>> _baseGeneralMap = {
    ...
    'Додатковий Дохід': ['фриланс', 'проєкт', 'гонорар', 'халтура', 'підробіток', 'продаж', 'кешбек', 'cashback'],
