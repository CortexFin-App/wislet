...
Future<Response> onRequest(RequestContext context) async {
  ...
  if (context.request.method == HttpMethod.get) {
    final walletId = int.tryParse(context.request.url.queryParameters['walletId'] ?? '');
    ...
    final data = await supabase.from('plans').select('*, categories(*)').eq('wallet_id', walletId);
  ...
  if (context.request.method == HttpMethod.post) {
    ...
    final data = await supabase.from('plans').insert(body).select().single();
