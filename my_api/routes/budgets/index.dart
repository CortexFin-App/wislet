...
Future<Response> onRequest(RequestContext context) async {
  ...
  if (context.request.method == HttpMethod.get) {
    ...
    if (walletId == null) {
      return Response(statusCode: HttpStatus.badRequest, body: 'walletId is required');
    }

    final isMember = await supabase.isUserMember(user.id, walletId);
    if (!isMember) return Response(statusCode: HttpStatus.forbidden);

    final data = await supabase.from('budgets').select().eq('wallet_id', walletId);
  ...
  if (context.request.method == HttpMethod.post) {
    ...
    final data = await supabase.from('budgets').insert(body).select().single();
